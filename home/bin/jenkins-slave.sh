...
if [[ $# -lt 1 ]] || [[ "$1" == "-"* ]]; then
  ...
  if [[ "$@" != *"-master "* ]] && [ ! -z "$JENKINS_PORT_8080_TCP_ADDR" ]; then
	PARAMS="-master http://$JENKINS_SERVICE_HOST:$JENKINS_SERVICE_PORT -tunnel $JENKINS_SLAVE_SERVICE_HOST:$JENKINS_SLAVE_SERVICE_PORT -username ${master_username} -passwordEnvVariable MASTER_PASSWORD -executors ${slave_executors}"
  fi
